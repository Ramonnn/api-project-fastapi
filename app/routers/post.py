...
def create_posts(
    ...
    new_post = models.Post(owner_id=current_user.id, **post.dict())  # Unpack post dict
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
...
def update_post(
    ...
    post_query.update(
        dict(**post.dict()), synchronize_session=False
    )  # pyright errors out when not repacking dict
